...
(function (ns) {
        ...
        var displayError = function (errorCode) {
            ...
            switch(errorCode) {
                case 'AUDIO_INACTIVITY_ERROR':
                    $j('#recording-message'+' > #error-msg').append('No activity. The recorder has been stopped.');
                    break;
                case 'SERVER_CONNECTION_ERROR':
                    $j('#recording-message'+' > #error-msg').append('The recorder is not available.');
                    break;
                case 'MISSING_MICROPHONE_ERROR':
                    $j('#recording-message'+' > #error-msg').append('You are missing a microphone.');
                    break;
                case 'MICROPHONE_PERMISSION_ERROR':
                    $j('#recording-message'+' > #error-msg').append('Microphone blocked in browser settings.');
                    break;
                case 'UNSUPPORTED_BROWSER_ERROR':
                    $j('#recording-message'+' > #error-msg').append('This browser does not support the recorder.');
                    break;
                case 'MAX_AUDIO_LENGTH_ERROR':
                    $j('#msgSentFail').text('You have reached the max audio length.');
                    break;
                default:
                    $j('#recording-message'+' > #error-msg').append('The recorder is not available.');

            }
            if ( errorCode !== 'MAX_AUDIO_LENGTH_ERROR' )  {
    ...
    ns.RecordingTeacherController = function () {
        var execute;
    ...
    ns.doneRecording = function (completed) {
        var d = $j.Deferred();
